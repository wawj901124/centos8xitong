...
#官网：https://pypi.org/project/requests-html/
#requests_html教程网址：https://www.jianshu.com/p/380974ba9540
...
class RunSpiderHanMan(object):
    ...
    #根据网址和title列表保存包数据库中
    def save_url_and_title_dict_to_db(self,url_and_title_dict):
        if url_and_title_dict == {}:
            print("无内容，不保存")
        else:
            for key, value in url_and_title_dict.items():
                url = key
                url_title = value
                from spiderdata.modelsurl import SpiderWebUrl
                #保存前先查看是否存在网址
                already_have_list = SpiderWebUrl.objects.filter(web_url=url)
                already_have_list_count = already_have_list.count()
                if already_have_list_count == 0: #说明不存在，则保存
                    new_spiderweburl = SpiderWebUrl()
                    new_spiderweburl.web_url = url
                    new_spiderweburl.url_title = url_title
                    new_spiderweburl.save()
                    print("保存一条数据完成")
                else:
                    print("数据已经存在，无需保存")
    ...
    #根据数据库中可以爬去的网址爬去新的网址
    def get_new_url_and_title_dict_from_db(self):
        url_and_title_dict = {}
        from spiderdata.modelsurl import SpiderWebUrl
        #获取到可以爬去但是还没有爬去的数据
        spider_url_and_title_list = SpiderWebUrl.objects.filter(is_spider=True).\
            filter(is_already_spider=False)
        spider_url_and_title_list_count = spider_url_and_title_list.count()
        if spider_url_and_title_list_count==0: #说明没有符合条件的数据，不爬取
            print("没有符合条件的数据，不爬取")
        else: #否则，爬取
            for spider_url_and_title_one in spider_url_and_title_list:
                m_w_u = spider_url_and_title_one.modify_web_url
                m_w_u = m_w_u.strip()
                if m_w_u ==None or m_w_u =="":
                    url = spider_url_and_title_one.web_url
                else:
                    url = m_w_u

                #根据url获取url页面的所有绝对路径作为url_list
                url_response = self.spider_common.getUrlResponse(hs=HTMLSession(), url=url)
                #获取所有链接绝对路径
                new_url_list = self.spider_common.get_response_absolute_links(url_response)
                #根据新得到的网址，获取网址对应的标题，并组合为字典
                # 根据url列表获取对应的网址和网址标题
                url_and_title_dict = self.spider_common.get_url_and_title_dict(url_list=new_url_list)
                # 保存网址和网址标题
                self.save_url_and_title_dict_to_db(url_and_title_dict=url_and_title_dict)

                #将已经爬去的网址置为已经爬取过
                spider_url_and_title_one.is_already_spider = True
                spider_url_and_title_one.save()  #更改为已经爬取
...
if __name__ == "__main__":
    ...
    # rshm.run()
    # yuming = "https://www.mhzzz.xyz"
    # weburl = "https://www.mhzzz.xyz/manhua/info/1.html"
    # shh = SpiderHanMan(yuming,weburl)
    # no_heade =  shh.get_response_no_header()
    # # shh.get_respinse_title(no_heade)
    # # print(no_heade.text)
    # url_list = shh.get_response_absolute_links(no_heade)
    # shh.get_url_and_title_dict(url_list)
    # print(no_heade.html.title)
    # save_dict = {'https://3000jl.com': '3000ä½³ä¸½-ä¸\x89å\x8d\x83ä½³ä¸½', 'http://www.baoma.buzz': '', 'https://tpgfulidh.com': 'è\x88\x94æ\x89¹å\x8c\xa0å¯¼è\x88ª-æ\x9c\x80æ\x96°ç½\x91å\x9d\x80å\x8f\x91å¸\x83', 'http://dxjdh001.com': '&#20892;&#22827;&#23548;&#33322;&#22823;&#20840;&#26368;&#26032;&#32593;&#22336;&#21457;&#24067;&#39029;&#95;&#31532;&#19968;&#31119;&#21033;&#23448;&#26041;&#23548;&#33322;&#32593;&#22336;&#20837;&#21475;', 'https://www.xmddh1.xyz': '小迷弟导航_福利视频_福利网站_福利导航_最新地址发布页', 'https://www.mdrkj.com/xxfldh/': 'å°\x8fXç¦\x8få\x88©å¯¼è\x88ª - å°\x8fXå¯¼è\x88ª,è\x80\x81å\x8f¸æ\x9cºç¦\x8få\x